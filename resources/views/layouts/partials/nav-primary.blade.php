...
<div class="space-y-1.5">
    ...
    <a href="{{ route('clients.index') }}"
       class="{{ $itemBase }} {{ $onClients ?? false ? $itemActive : $itemIdle }}">
        <span class="absolute left-0 top-2.5 bottom-2.5 w-[3px] rounded-full
                     {{ ($onClients ?? false) ? 'bg-[color:var(--tw-accent)]' : 'bg-transparent' }}"></span>

        <span class="{{ $iconWrapBase }} {{ ($onClients ?? false) ? $iconWrapActive : $iconWrapIdle }}">
            <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
