    ...
    <a href="{{ route('transporters.index') }}"
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-xs text-slate-400 mb-0.5">Outstanding Freight Payables</p>
            @if($totalFreightPayable > 0)
              <p class="text-2xl font-bold text-amber-400">${{ number_format($totalFreightPayable, 2) }}</p>
            @else
              <p class="text-2xl font-bold text-slate-300">$0.00</p>
            @endif
          </div>
        </div>
        <svg class="w-4 h-4 text-slate-600 group-hover:text-amber-400 transition-colors" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <div class="mt-4 border-t border-slate-800 pt-4 space-y-2">
          @foreach($topTransporters as $t)
            <div class="flex items-center justify-between text-sm">
              ...
              <span class="font-semibold text-amber-300">${{ number_format($t->balance, 2) }}</span>
            </div>
          @endforeach
