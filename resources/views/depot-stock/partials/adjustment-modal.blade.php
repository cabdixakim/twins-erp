{{-- resources/views/depot-stock/partials/adjustment-modal.blade.php --}}
@php
    ...
    <div class="w-full max-w-lg rounded-2xl border {{ $border }} {{ $surface }} shadow-xl overflow-hidden">
      <div class="p-5 border-b {{ $border }} {{ $surface2 }}">
        <div class="flex items-start justify-between gap-4">
          <div class="min-w-0">
            <div class="text-base font-semibold {{ $fg }}">Adjustment</div>
            <div class="mt-1 text-xs {{ $muted }}">
              Create a manual stock correction for <span class="{{ $fg }} font-semibold">{{ $currentDepot->name }}</span>.
              (Wiring comes next.)
            </div>
          </div>
      ...
      <div class="p-5">
        <div class="rounded-xl border {{ $border }} {{ $surface2 }} p-4">
          <div class="text-sm font-semibold {{ $fg }}">Not wired yet</div>
          <div class="mt-1 text-xs {{ $muted }}">
