    ...
    <div class="flex items-center justify-between gap-3">
        <div class="min-w-0">
            ...
            <div class="flex items-center gap-2">
                <h2 class="text-sm font-semibold truncate">{{ $transporter->name }}</h2>
                <span class="inline-flex items-center rounded-full px-2 py-0.5 text-[10px]
                    {{ $transporter->is_active ? 'bg-emerald-500/10 text-emerald-300 border border-emerald-500/50'
        ...
        <form method="post" action="{{ route('settings.transporters.update', $transporter) }}" class="space-y-3">
            ...
            <div>
                ...
                <textarea name="notes" rows="2"
                          class="w-full rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-sm focus:outline-none focus:ring focus:ring-emerald-500/40">{{ old('notes', $transporter->notes) }}</textarea>
            </div>
